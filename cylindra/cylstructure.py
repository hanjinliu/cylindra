...
def with_rise_angle(mole: Molecules, spl: CylSpline) -> pl.DataFrame:
    ...
    for _, sub in _groupby_with_index(mole.concat_with(mole_ext), Mole.nth):
        ...
        subsets.append(
            sub.with_features(pl.Series(Mole.rise, rise_angles).fill_nan(-np.inf))
        )
    return _concat_groups(subsets).features


def with_lateral_interval(
    mole: Molecules, spl: CylSpline, projective: bool = True
) -> pl.DataFrame:
    _spl_len = spl.length()
    subsets = list[Molecules]()
    ...
    for _, sub in _groupby_with_index(mole.concat_with(mole_ext), Mole.nth):
        ...
        subsets.append(
            sub.with_features(
                pl.Series(Mole.lateral_interval, _interv_abs).fill_nan(-np.inf)
            )
        )
    return _concat_groups(subsets).features

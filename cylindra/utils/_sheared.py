...
def pad_sheared_edges(
    ...
    if pad_long < start:
        ...
        lpad_padded = np.pad(
            lpad, pad_width=[(pad_long + start, pad_long - start), (0, 0)], **pad_kwargs
        )
        rpad_padded = np.pad(
            rpad, pad_width=[(pad_long - start, pad_long + start), (0, 0)], **pad_kwargs
        )
...
def interval_filter(
    pos: np.ndarray, vec: np.ndarray, filter_length: int, filter_width: int, start: int
) -> np.ndarray:

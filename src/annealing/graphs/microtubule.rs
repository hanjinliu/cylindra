...
use crate::{
    ...
    hash::HashMap2D,
    annealing::{
        potential::{MicrotubulePotential, MicrotubulePotentialLJ, MicrotubuleBindingPotential, EdgeType},
...
/// A graph representing a microtubule lattice.
///
/// Unlike `CylindricalGraph`, longitudinal edges (running along a protofilament) do
/// not directly carry an angle constraint. Instead, every node that has both a
/// "previous" and a "next" longitudinal neighbor (i.e. every node that is not at a
/// protofilament tip) contributes a deforming (bending) energy computed from the
/// triplet of positions, using `MicrotubuleBindingPotential::calculate_deform`. That
/// energy is asymmetric: protofilaments are allowed to curl outward (away from the
/// microtubule axis) but not inward, which is the hallmark of a microtubule tip.
#[derive(Clone)]
pub struct MicrotubuleGraph<T: MicrotubuleBindingPotential> {
...
impl<T> MicrotubuleGraph<T> where T: MicrotubuleBindingPotential {
    ...
    /// Find the longitudinal neighbors of node `idx` that lie on the same
    /// protofilament, split into the one with a smaller `y` (prev) and the one with
    /// a larger `y` (next). Either (or both) can be `None` if `idx` is at (or next
    /// to) a protofilament tip.
    fn longitudinal_neighbors(&self, idx: usize) -> (Option<usize>, Option<usize>) {
        let graph = self.components();
        let node = graph.node_state(idx);
        let mut prev = None;
        let mut next = None;
        for k in graph.connected_edge_indices(idx) {
            if graph.edge_state(*k) != &EdgeType::Longitudinal {
                continue;
            }
            let ends = graph.edge_end(*k);
            let other_idx = if ends.0 == idx { ends.1 } else { ends.0 };
            let other = graph.node_state(other_idx);
            if other.index.y < node.index.y {
                prev = Some(other_idx);
            } else {
                next = Some(other_idx);
            }
    ...
    /// Calculate the deforming (bending) energy of the triplet
    /// (`node_state_prev`, `node_state`, `node_state_next`), where `node_state` is
    /// the center of the triplet. The outward-pointing normal (`ez`) of the center's
    /// coordinate system is used to tell curling outward from curling inward.
    fn deforming(
        &self,
        node_state_prev: &Node2D<Shift>,
        node_state: &Node2D<Shift>,
        node_state_next: &Node2D<Shift>,
    ) -> f32 {
        let idx = &node_state.index;
        let idx1 = &node_state_prev.index;
        let idx2 = &node_state_next.index;
        let coord = &self.coords[(idx.y, idx.a)];
        let coord1 = &self.coords[(idx1.y, idx1.a)];
        let coord2 = &self.coords[(idx2.y, idx2.a)];
    ...
    /// Calculate the deforming energy centered at node `idx`, using its current
    /// state `state`. Returns 0.0 if `idx` is at a protofilament tip.
    fn deforming_at(&self, idx: usize, state: &Node2D<Shift>) -> f32 {
        match self.longitudinal_neighbors(idx) {
            (Some(prev), Some(next)) => {
                let graph = self.components();
                let state_prev = graph.node_state(prev);
                let state_next = graph.node_state(next);
                self.deforming(state_prev, state, state_next)
            }
            _ => 0.0,
        }
    }

    /// Calculate the total energy of the graph, including the internal, binding and
    /// deforming (bending) energies.
    pub fn energy(&self) -> f32 {
        ...
        for i in 0..graph.node_count() {
            energy += self.deforming_at(i, &graph.node_state(i));
        }
        energy
    }
}

impl MicrotubuleGraph<MicrotubulePotential> {
    ...
    pub fn empty() -> Self {
        Self {
            ...
            binding_potential: MicrotubulePotential::unbounded(),
            local_shape: Vector3D::new(0, 0, 0),
        }
    }
}

impl MicrotubuleGraph<MicrotubulePotentialLJ> {
    /// Create a graph with no nodes or edges.
    pub fn empty() -> Self {
        Self {
            components: GraphComponents::empty(),
            coords: Arc::new(HashMap2D::new()),
            energy: Arc::new(HashMap2D::new()),
            binding_potential: MicrotubulePotentialLJ::unbounded(),
            local_shape: Vector3D::new(0, 0, 0),
        }
    }
}

impl<T> GraphTrait<Node2D<Shift>, EdgeType> for MicrotubuleGraph<T> where T: MicrotubuleBindingPotential {
    ...
    /// Calculate the binding energy between two nodes. Unlike `CylindricalGraph`,
    /// this does not include any angle constraint: longitudinal binding only
    /// depends on the distance, and the curvature/bending constraint is handled
    /// separately by `deforming`.
    /// # Arguments
    ...
    fn binding_old_new(
        ...
        let (e_old, e_new) = match typ {
            EdgeType::Longitudinal => (
                ...
                self.binding_potential.calculate_bind(&dr_new),
            ),
            EdgeType::Lateral => (
                self.binding_potential.calculate_lat_bind(&dr_old),
                self.binding_potential.calculate_lat_bind(&dr_new),
            ),
        };
        (e_old, e_new)
    }
    ...
    /// Energy difference by shifting a state of node at idx. In addition to the
    /// binding energy of the connected edges, this also accounts for the change in
    /// deforming energy of every triplet that involves node `idx`: the triplet
    /// centered at `idx` itself, and the triplets centered at its longitudinal
    /// prev/next neighbors (since `idx` is one of the endpoints of those triplets).
    fn energy_diff_by_shift(
        ...
        // at `idx` itself (if it is not at a protofilament tip), and the ones
        // centered at its prev/next neighbors (in which `idx` plays the role of the
        // *other* endpoint). Note that `idx` need not have both a prev and a next
        // for the latter two to apply -- e.g. if `idx` is a tip with only a `next`,
        // shifting it still changes the triplet centered at that `next` neighbor.
        let (prev_opt, next_opt) = self.longitudinal_neighbors(idx);

        if let (Some(prev), Some(next)) = (prev_opt, next_opt) {
            let state_prev = graph.node_state(prev);
            let state_next = graph.node_state(next);
        ...
        if let Some(prev) = prev_opt {
            if let (Some(prev_prev), Some(_)) = self.longitudinal_neighbors(prev) {
                let state_prev_prev = graph.node_state(prev_prev);
                let state_prev = graph.node_state(prev);
                e_old += self.deforming(state_prev_prev, state_prev, state_old);
                e_new += self.deforming(state_prev_prev, state_prev, state_new);
            }
        }
        if let Some(next) = next_opt {
            if let (Some(_), Some(next_next)) = self.longitudinal_neighbors(next) {
                let state_next = graph.node_state(next);
                let state_next_next = graph.node_state(next_next);
                e_old += self.deforming(state_old, state_next, state_next_next);
                e_new += self.deforming(state_new, state_next, state_next_next);
            }
        }
        e_new - e_old
    }
...
impl<T> CylindricGraphTrait<Shift, EdgeType> for MicrotubuleGraph<T> where T: MicrotubuleBindingPotential {
...
impl<T> MicrotubuleGraph<T> where T: MicrotubuleBindingPotential {
    /// Deforming (bending) energy contributed by each node.
    fn deform_energies(&self) -> Array1<f32> {
        let graph = self.components();
        let mut engs = Array1::zeros(graph.node_count());
        for i in 0..graph.node_count() {
            engs[i] = self.deforming_at(i, &graph.node_state(i));
        }
        engs
    }
}

...
use crate::annealing::{
    graphs::{CylindricGraphTrait, MicrotubuleGraph, GraphTrait},
    potential::{MicrotubulePotential, MicrotubulePotentialLJ},
    random::RandomNumberGenerator, reservoir::Reservoir
};
use crate::{value_error, cylindric::Index};
use super::misc::OptimizationState;

#[pyclass]
/// A class to perform simulated annealing on a microtubule lattice.
///
/// Unlike `CylindricAnnealingModel`, the longitudinal (protofilament) bending
/// potential is asymmetric: protofilaments can curl outward (away from the
/// microtubule axis) but are strongly restricted from curling inward, mimicking
/// the mechanical behavior of a real microtubule tip.
pub struct MicrotubuleAnnealingModel {
    rng: RandomNumberGenerator,
    optimization_state: OptimizationState,
    graph: MicrotubuleGraph<MicrotubulePotential>,
    reservoir: Reservoir,
    iteration: usize,
    reject_limit: usize,
}
...
#[pymethods]
impl MicrotubuleAnnealingModel {
    #[new]
    ...
    #[pyo3(signature =
        (lon_dist_min, lon_dist_max, lat_dist_min, lat_dist_max, lon_ang_min=0.0, lon_ang_max=-1.0, cooling_rate=1e-3))
    ]
    /// Set a box potential with given borders. `lon_ang_min` and `lon_ang_max` define
    /// the allowed range, in radians, of the (signed) protofilament bending angle: a
    /// positive angle means the protofilament curls outward (away from the
    /// microtubule axis), a negative angle means it curls inward. `lon_ang_min` should
    /// usually stay close to 0 so inward curling is essentially forbidden, while
    /// `lon_ang_max` can be set larger to softly allow outward curling near tips.
    pub fn set_box_potential(
        ...
        let model = slf.graph.binding_potential
            ...
            .with_lon_ang(lon_ang_min, lon_ang_max)?
...
// ----------------------------------------------------------
// Microtubule annealing model with Lennard-Jones-like potential
// ----------------------------------------------------------

#[pyclass]
/// A class to perform simulated annealing on a microtubule lattice, using a
/// Lennard-Jones-like distance boundary instead of `MicrotubuleAnnealingModel`'s hard
/// trapezoidal one. The longitudinal bending potential stays asymmetric, exactly as
/// in `MicrotubuleAnnealingModel`.
pub struct MicrotubuleAnnealingModelLJ {
    rng: RandomNumberGenerator,
    optimization_state: OptimizationState,
    graph: MicrotubuleGraph<MicrotubulePotentialLJ>,
    reservoir: Reservoir,
    iteration: usize,
    reject_limit: usize,
}
...
#[pymethods]
impl MicrotubuleAnnealingModelLJ {
    #[new]
    #[pyo3(signature = (seed=0))]
    pub fn new(seed: u64) -> Self {
        let rng = RandomNumberGenerator::new(seed);
    ...
    /// Set a box potential with given borders. See `MicrotubuleAnnealingModel.set_box_potential`
    /// for the meaning of `lon_ang_min`/`lon_ang_max`.
    pub fn set_box_potential(
        mut slf: PyRefMut<Self>,
        lon_dist_min: f32,
        lon_dist_max: f32,
    ...
    pub fn set_energy_inf(&mut self, lon_energy_inf: f32, lat_energy_inf: f32) -> PyResult<()> {
        let model = self.graph.binding_potential.with_energy_inf(lon_energy_inf, lat_energy_inf)?;
        self.graph.set_potential_model(model);
        Ok(())
    }
